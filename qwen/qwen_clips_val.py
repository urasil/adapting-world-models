# val runner for qwen/qwen_clips.py (clips-as-context), analogous to qwen/qwen_val.py (text-based context).
...
from qwen.qwen_clips import (          # noqa: E402
...
def main():
    ...
    ap.add_argument("--prior-mode", choices=PRIOR_MODES, default="clips",
        help="'none': only the target clip. 'clips': prior clips as video (no labels). "
             "'clips_labelled': prior clips with correct/mistake labels (oracle).")
    ap.add_argument("--reasoning-mode", choices=REASONING_MODES, default="direct",
        help="'direct': immediate prediction. 'cot': chain-of-thought.")
    ap.add_argument("--max-prior-clips", type=int, default=None,
        help="Cap on the number of most-recent prior clips to include (default: all).")
    ap.add_argument("--fps", type=float, default=8.0, help="Frames per second to sample from each clip (default 2.0).")
    ap.add_argument("--force", action="store_true", help="Ignore existing output file and rerun everything.")
    ap.add_argument("--max-videos", type=int, default=None, help="Only process this many videos (debug / smoke test).")
    args = ap.parse_args()
    ...
    for i, (video_name, entry, n_expected) in enumerate(todo, 1):
        ...
        try:
            results = process_video(video_path, entry, processor, model, prior_mode=args.prior_mode,
                reasoning_mode=args.reasoning_mode, fps=args.fps, max_prior_clips=args.max_prior_clips, test=False)
        ...
        print(f"  done in {v_dt:.1f}s  ({len(results)} examples, {v_dt / max(len(results), 1):.2f}s/example)")
        print(f"  cumulative: {egs_done} examples in {elapsed / 60:.1f} min ({rate:.2f} ex/s)")
