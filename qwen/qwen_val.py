# runs the qwen3-vl zero-shot mistake-detection baseline on holoassist val.
...
from qwen.qwen_baseline import load_model, process_video, PRIOR_MODES, REASONING_MODES  # noqa: E402
...
def main():
    ...
    ap.add_argument('--prior-mode',  choices=PRIOR_MODES, default='actions_with_labels',
                    help="Passed through to process_video. See qwen/qwen_baseline.py.")
    ap.add_argument('--reasoning-mode', choices=REASONING_MODES, default='direct',
                    help="Prompting strategy: 'direct' for immediate prediction, 'cot' for chain-of-thought.")
    ...
    for i, (video_name, entry, n_expected) in enumerate(todo, 1):
        ...
        try:
            results = process_video(video_path, entry, processor, model,
                prior_mode=args.prior_mode, reasoning_mode=args.reasoning_mode, test=False)
