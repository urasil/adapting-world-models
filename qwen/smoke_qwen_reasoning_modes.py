...
# cot prompts, then runs qwen/qwen_baseline.py and qwen/qwen_val.py twice each on a single-video
# subset, to exercise the shared prompt/inference path without the full dataset.
...
from qwen.qwen_baseline import build_prompt, group_fine_by_coarse
...
def pick_example(annotations: list[dict], video_dir: Path, video_name: str | None) -> tuple[dict, dict, list[dict]]:
    # returns (entry, coarse, members) for the first usable example
    if video_name:
...
def main() -> None:
    ...
    target_parts = (str(target_attrs.get("Verb", "")).strip(), str(target_attrs.get("Adjective", "")).strip(),
                    str(target_attrs.get("Noun", "")).strip())
    target_desc = " ".join(part for part in target_parts if part) or "(unspecified action)"
        ...
        for script_name in ("qwen_baseline.py", "qwen_val.py"):
            for reasoning_mode in ("direct", "cot"):
                ...
                if script_name == "qwen_baseline.py":
                    command = [
                        sys.executable,
                        str(REPO_ROOT / "qwen" / script_name),
