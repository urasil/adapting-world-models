...
def run_encoder(processor, model, dtype, frames_tchw: torch.Tensor) -> np.ndarray:
    # runs V-JEPA 2.1 on a (T, C, H, W) uint8 clip, returns FULL token features (32, 576, 1024) f32 -- no spatial pooling, caller decides
...
def _norm_rows(x: np.ndarray) -> np.ndarray:
    # L2-normalises the last axis
    return x / (np.linalg.norm(x, axis=-1, keepdims=True) + 1e-8)

# full-token metrics

def full_token_cosine_stats(a: np.ndarray, b: np.ndarray) -> dict:
    # compares every spatial token independently for a, b: (32, 576, 1024) f32, returns mean/std/min over all 18432 per-token cos-sims plus per_t_mean, centre_mean (inner 22x22, 84%), edge_mean (1-token border, 16%)
...
def pooled_cosine_sim_per_t(a: np.ndarray, b: np.ndarray) -> np.ndarray:
    # spatially pools first, then compares per temporal position: (32, 576, 1024) -> pool -> (32, 1024) -> cosine sim -> (32,)
...
def pooled_global_cosine_sim(a: np.ndarray, b: np.ndarray) -> float:
    # cosine sim between spatially-pooled global vectors, matches the stored .npz format
...
def find_processable_segments(
    ...
    # up to max_segments segments with the given label (0=correct, 1=mistake) from videos where both a .npz feature file and the source video exist, each dict has video_name, seg_idx, label, frame_indices, start/end_sec, verb, noun, stored_feats (32,1024) f32, video_path
    results = []
...
def process_segment(seg: Dict, processor, model, dtype, aug_names: List[str], rng: random.Random) -> Dict:
    # for one segment: re-decode original frames, encode -> full (32,576,1024) features, then per augmentation apply+encode and compute both FULL (per-token) and POOLED (spatial mean-pool first) metric families
    t0 = time.time()
...
def compute_pairwise_baselines(
    ...
    # cosine similarity between pairs of different segments (same label), using stored features (no re-encoding, fast), returns [{seg_a, seg_b, cos_sim_global, l2_dist_global}, ...]
    results = []
...
def parse_args():
    p = argparse.ArgumentParser(description="Test V-JEPA 2.1 feature sensitivity to mild augmentations.")
    p.add_argument("--features_dir", default="features/vjepa2_train", help="Directory with .npz feature files")
    p.add_argument("--video_dir", default="holoassist/videos", help="Root directory for HoloAssist videos")
    p.add_argument("--output", default="results/augmentation_sensitivity.json", help="Path to write JSON results")
    p.add_argument("--n_segments", type=int, default=30, help="Number of mistake segments to augment+encode (default 30)")
    p.add_argument("--n_baseline", type=int, default=20, help="Number of pairwise baseline comparisons per condition (default 20)")
    p.add_argument("--n_correct", type=int, default=30, help="Number of correct segments to load for cross-class baseline")
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--augmentations", nargs="+", default=list(AUG_REGISTRY.keys()), choices=list(AUG_REGISTRY.keys()),
    ...
    p.add_argument("--crop_frac", type=float, default=None,
                   help="Override crop_frac for slight_crop and combined augmentations "
                        "(default: use registry value, currently 0.70). E.g. --crop_frac 0.9 for a milder 90%% crop.")
    p.add_argument("--smoke", action="store_true", help="Quick test: 3 segments, 3 baseline pairs, skips combined aug")
    return p.parse_args()
...
def main():
    ...
    mistakes = find_processable_segments(features_dir, video_dir, label=1, max_segments=args.n_segments, rng=rng)
    ...
    corrects = find_processable_segments(features_dir, video_dir, label=0, max_segments=args.n_correct, rng=rng)
    ...
    mistake_pairs = compute_pairwise_baselines(mistakes, processor, model, dtype, args.n_baseline, rng, label=1)
    correct_pairs = compute_pairwise_baselines(corrects, processor, model, dtype, args.n_baseline, rng, label=0)
    ...
    print("\nRun  python evaluation/analyse_augmentation_sensitivity.py --results", out_path,
