# builds a summary figure from augmentation_sensitivity.json. cpu-only, runs on the login node.
# usage: python plot_augmentation_results.py --results results/augmentation_sensitivity.json
...
def seg_sims(data: dict, aug: str) -> list[float]:
    return [s["augmentations"][aug]["cos_sim_global"]
...
def l2_sims(data: dict, aug: str) -> list[float]:
    return [s["augmentations"][aug]["l2_dist_global"]
            for s in data["segments"] if aug in s.get("augmentations", {})]
