...
from qwen_baseline import load_model, process_video, PRIOR_MODES, REASONING_MODES  # noqa: E402
...
def main():
    ...
    ap.add_argument('--prior-mode',  choices=PRIOR_MODES, default='actions_with_labels',
                    help="Passed through to process_video. See qwen_baseline.py.")
    ...
    for i, (video_name, entry, n_expected) in enumerate(todo, 1):
        ...
        print(f"  done in {v_dt:.1f}s  ({len(results)} examples, "
              f"{v_dt / max(len(results), 1):.2f}s/example)")
        print(f"  cumulative: {egs_done} examples in {elapsed/60:.1f} min "
              f"({rate:.2f} ex/s)")
