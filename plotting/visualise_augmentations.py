"""
Decodes one real HoloAssist frame and saves original vs each augmentation
side by side. CPU-only, runs on the login node in seconds.

Usage:
    python plotting/visualise_augmentations.py [--video NAME --frame N] [--output results/aug_comparison.png]
"""
...
def decode_one_frame(video_path: str, frame_idx: int) -> np.ndarray:
    # returns (H, W, 3) uint8 RGB array for the closest decoded frame
...
def draw_crop_box(ax, H: int, W: int, crop_frac: float, top: int, left: int):
    # overlays the actual crop window on an axis
