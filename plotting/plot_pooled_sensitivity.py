"""
Same as plot_full_token_sensitivity.py but for spatially-pooled features - the
format the downstream classifier actually trains on. The panel layout mirrors
that script's figure exactly, so the two are easy to compare side by side.

Usage:
    python plot_pooled_sensitivity.py --results results/augmentation_sensitivity_full_tokens_crop90.json \
...
def styled_boxplot(ax, data_list, positions, colours, rng):
    # box plots with individual jittered points -- mirrors plotting/plot_full_token_sensitivity.py
    bp = ax.boxplot(
        data_list,
        positions=positions,
        widths=0.35,
