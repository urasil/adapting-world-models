...
def run_inference(
    ...
    fps: float = 2.0, min_frames: int = 4, max_frames: int = 32, max_new_tokens: int = 512
) -> str:
...
def process_video(video_path: str, annotation_entry: dict, processor, model, prior_mode: str = "actions_with_labels", reasoning_mode: str = "direct", test: bool = False) -> List[dict]:
    """
    For every coarse action, predict each sub-action's correctness using only
    the target segment's frames as visual input, plus an optional text
    description of prior sub-actions.
    """
        ...
        for k_idx, target in enumerate(members):
            ...
            try:
                response = run_inference(
                    processor, model, video_path, prompt,
                    video_start=video_kwargs["video_start"],
                    video_end=video_kwargs["video_end"],
                    fps=video_kwargs["fps"],
                    min_frames=video_kwargs["min_frames"],
                    max_frames=video_kwargs["max_frames"],
                )
