...
# done for the world model approach.
# usage: python enrich_index_with_actions.py --train_index ... --val_index ...
#   --train_features_dir ... --val_features_dir ... --output_dir datasets
import argparse
import json
import os
...
def main():
    ...
    parser.add_argument("--train_features_dir", required=True, help="dir of per-video .npy + _meta.npz files (train)")
    parser.add_argument("--val_features_dir",   required=True, help="dir of per-video .npy + _meta.npz files (val)")
    ...
    features_dirs = [args.train_features_dir, args.val_features_dir]
    ...
    train_enriched, train_miss = _enrich(train_idx["examples"], features_dirs, verb_vocab, noun_vocab)
    print(f"  {len(train_enriched)} examples, {train_miss} missing metadata")

    print("Enriching val…  (val-only verbs/nouns → id=0/__unknown__)")
    val_enriched, val_miss = _enrich(val_idx["examples"], features_dirs, verb_vocab, noun_vocab)
    print(f"  {len(val_enriched)} examples, {val_miss} missing metadata")
    ...
    def _save(examples, orig, features_dir, fname):
        out = {
            "features_dirs": [str(Path(features_dir).resolve())],
    ...
    _save(train_enriched, train_idx, args.train_features_dir, "wm_train_index.json")
    _save(val_enriched,   val_idx,   args.val_features_dir,   "wm_val_index.json")
    print("Done.")
