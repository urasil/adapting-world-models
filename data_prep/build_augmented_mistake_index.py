...
def class_counts(examples: list[dict]) -> tuple[int, int]:
    # returns (n_correct, n_mistake) for a list of example dicts
...
def make_augmented_example(orig: dict, aug_name: str) -> dict:
    # copy of orig with video_name suffixed by _aug_{aug_name}, all other fields kept verbatim so the feature loader picks up the augmented .npz
    return {**orig, "video_name": f"{orig['video_name']}_aug_{aug_name}"}

def check_aug_coverage(mistake_examples: list[dict], aug_features_dir: Path, aug_name: str) -> tuple[list[dict], list[str]]:
    # returns (coverable_examples, missing_video_names) for aug_name, coverable = augmented .npz exists on disk
    coverable = []
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Combine original + augmented mistake examples to reach target class ratio.")
    p.add_argument("--original_index", required=True, help="Path to the original dataset index JSON (e.g. datasets/mistake_detection_index.json).")
    p.add_argument("--aug_features_dir", required=True,
        help="Directory that contains the augmented .npz files produced by extract_augmented_features.py (e.g. features/vjepa2_train_aug).")
    p.add_argument("--aug_names", nargs="+", default=["warm_tint", "cold_tint", "crop_90", "warm_crop"],
        help="Augmentation variant names to consider (in priority order). Default: warm_tint cold_tint crop_90 warm_crop")
    p.add_argument("--output_path", required=True, help="Path to write the combined index JSON.")
    p.add_argument("--target_ratio", type=float, default=4.0,
        help="Desired n_correct / n_mistake ratio in the output index. Default 4 -> mistake class is 1/4 the correct class. Use 0 to include all available augmented examples.")
    p.add_argument("--dry_run", action="store_true", help="Print statistics and exit without writing any file.")
    return p.parse_args()
...
def main() -> None:
    ...
    orig_features_dir = idx.get("features_dir") or (idx.get("features_dirs", [None])[0])
    ...
    n_aug_needed = max(0, n_mistake_target - n_mistake_orig) if n_mistake_target is not None else None
    ...
    for aug_name in args.aug_names:
        coverable, missing = check_aug_coverage(mistake_examples, aug_features_dir, aug_name)
    ...
    combined = {
        ...
        "features_dirs": [str(orig_features_dir), str(aug_features_dir.resolve())],
