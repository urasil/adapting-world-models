...
def extract_and_pool(processor, model, dtype, frames_tchw: torch.Tensor) -> np.ndarray:
    # run V-JEPA 2.1 on a (T, C, H, W) uint8 clip, spatial mean-pool -> (T_TOKENS=32, EXPECTED_HIDDEN=1024) float16, matching the original .npz feature format
    ...
    pooled = raw.reshape(T_TOKENS, S_TOKENS, EXPECTED_HIDDEN).mean(axis=1)  # (32, 1024)
    return pooled.astype(np.float16)

# per-video processing

def process_video(npz_path: Path, video_path: Path, aug_fn, aug_kwargs: dict,
                   processor, model, dtype, base_rng_seed: int) -> dict | None:
    # per segment: decode frames from the stored indices, augment, encode, pool, returns None on a fatal error (e.g. corrupt video)
    # seeds are derived from base_rng_seed per segment so reruns reproduce the same augmentations
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Re-extract V-JEPA 2.1 features (spatially pooled) after augmentation.")
    p.add_argument("--features_dir", required=True,
        help="Directory containing the original per-video .npz feature files (e.g. features/vjepa2_train).")
    p.add_argument("--video_dir", required=True,
        help="Root directory for HoloAssist videos. Each video is at <video_dir>/<video_name>/Export_py/Video_compress.mp4.")
    p.add_argument("--output_dir", required=True,
        help="Directory for augmented .npz output files (e.g. features/vjepa2_train_aug). Created if absent.")
    p.add_argument("--aug_name", required=True, choices=list(AUG_REGISTRY.keys()),
        help=f"Augmentation to apply. Training variants: {TRAIN_AUG_NAMES}")
    p.add_argument("--seed", type=int, default=42,
        help="Base random seed for stochastic augmentations (e.g. crop jitter). Each segment gets a deterministic child seed: seed*10000 + video_idx*100 + seg_idx.")
    p.add_argument("--resume", action="store_true", default=True, help="Skip videos whose output .npz already exists (default: on).")
    p.add_argument("--no_resume", dest="resume", action="store_false", help="Overwrite existing output files.")
    return p.parse_args()
...
def main() -> None:
    ...
    for vi, npz_path in enumerate(npz_files):
        ...
        result = process_video(npz_path, video_path, aug_fn, aug_kwargs, processor, model, dtype, base_rng_seed)
