# cpu-only analysis of the json produced by test_augmentation_sensitivity.py.
# prints mean cos-sim between original/augmented features vs natural in/between-class
# variation, plus a per-augmentation safety verdict.
# usage: python analyse_augmentation_sensitivity.py --results results/augmentation_sensitivity.json [--plot]
...
def fmt_stat(d: dict, key: str = "cos_sim_global") -> str:
    """Format mean ± std (min–max) from a summary dict."""
...
def interpret(aug_mean: float, natural_mean: float) -> str:
    """Choose a verdict based on aug_mean cos-sim."""
...
def _collect_aug_sims(segments: List[Dict], aug_name: str) -> List[float]:
    return [
        s["augmentations"][aug_name]["cos_sim_global"]
        for s in segments
...
def main():
    ...
    p.add_argument("--results", required=True,
                   help="Path to results JSON from test_augmentation_sensitivity.py")
    p.add_argument("--plot", action="store_true",
                   help="Save a bar chart PNG alongside the results JSON")
    args = p.parse_args()

    results_path = Path(args.results)
    if not results_path.exists():
        print(f"ERROR: {results_path} not found. Run test_augmentation_sensitivity.py first.")
        sys.exit(1)
