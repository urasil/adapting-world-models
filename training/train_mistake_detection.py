...
# (~4%) and we want to up-weight them
...
def load_split_file(path: str) -> set[str]:
    # loads a HoloAssist split file: one video name per line, possibly with comments (#...) and blank lines
    videos = set()
    with open(path) as f:
        for line in f:
...
def official_video_split(dataset: HoloMistakeDataset, train_split_file: str, val_split_file: str) -> tuple[List[int], List[int]]:
    # partitions by the official train/val split files, examples whose video_name is in neither file are dropped with a warning (index built from a superset, or a naming mismatch)
...
def video_split(dataset: HoloMistakeDataset, val_frac: float = 0.1, seed: int = 0) -> tuple[List[int], List[int]]:
    # splits by video name, not by example -- adjacent sliding-window prefixes share most content, so a per-example split would leak near-duplicates
...
def subsample_negatives(dataset: HoloMistakeDataset, indices: List[int], neg_to_pos_ratio: float, seed: int = 0) -> List[int]:
    # per-video stratified negative subsampling: keep all positives + up to neg_to_pos_ratio x n_pos negatives per video (fewer negatives than quota -> keep them all, no positives -> no examples)
    rng = random.Random(seed)
...
def filter_excluded_tasks(dataset: HoloMistakeDataset, indices: List[int], exclude_tasks: List[str]) -> List[int]:
    # drops every example whose video belongs to one of exclude_tasks, removing whole tasks (vs. randomly subsampling) keeps every remaining task's data fully intact
...
def check_max_segments(dataset: HoloMistakeDataset, indices: List[int], max_segments: int) -> int:
    longest = 0
    for i in indices:
...
def validate_feature_coverage(dataset: HoloMistakeDataset, indices: List[int]) -> None:
    # fails fast if any selected video lacks a corresponding feature file
    missing_videos = []
    seen_videos = set()
    for i in indices:
...
class FocalLossBinary(nn.Module):
    # binary focal loss on logits (Lin et al., 2017): L = -alpha_t*(1-p_t)^gamma*log(p_t), alpha weights the POSITIVE class, so alpha=0.75 (not COCO's 0.25) to up-weight our rare ~4% mistake class
...
def cosine_lr(step: int, total_steps: int, base_lr: float,
    ...
    # half-cosine from base_lr to final_lr over total_steps -- equivalent to the paper's warmup-constant-cooldown schedule in the warmup=0, final_lr=0 limit (the probe setting)
    if step >= total_steps:
        return final_lr
...
def main():
    ...
    full = HoloMistakeDataset(args.index_path, features_dir=features_dir_override, max_segments=args.max_segments)
    ...
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, sampler=train_sampler, num_workers=args.num_workers,
        collate_fn=collate_fn, pin_memory=pin, drop_last=True, persistent_workers=args.num_workers > 0,
        prefetch_factor=2 if args.num_workers > 0 else None)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers,
        collate_fn=collate_fn, pin_memory=pin, persistent_workers=args.num_workers > 0,
        prefetch_factor=2 if args.num_workers > 0 else None)

    # ---- model ------------------------------------------------------------
    model = MistakeClassifier(embed_dim=args.embed_dim, num_heads=args.num_heads, depth=args.depth,
        max_segments=args.max_segments, tokens_per_segment=args.tokens_per_segment).to(device)
