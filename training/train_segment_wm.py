...
def warmup_constant_decay_lr(step: int, total_steps: int, *, warmup_lr: float, peak_lr: float,
    ...
    # warmup-constant-decay schedule (V-JEPA 2 Appendix B.1): linear warmup, hold, linear decay. Phase lengths are fixed fractions of total_steps matching the paper's 4500/85500/4500 split
...
def wm_train_split(dataset: SegmentWMDataset, train_file: str, val_file: str) -> list[int]:
    # positions in `dataset` whose video is in the official train split, dataset only holds train videos by construction, val split file is used only to check overlap and flag stray videos
...
def load_predictor_ckpt(model: nn.Module, ckpt_path: str, device) -> None:
    # loads predictor weights from a V-JEPA checkpoint (strict=False), handles a bare state-dict or a dict with a 'predictor'/'target_predictor'/'model' key
...
def save_checkpoint(path: Path, *, epoch, step, resume_iter, model, optimizer, args, best_val_l1):
    # saves to a temp file then renames, so a crash mid-write can never leave a truncated checkpoint at `path` (rename is atomic on the same filesystem)
    ckpt = {
...
def main():
    ...
    val_ds_full = SegmentWMDataset(args.val_index, train_mode=False,
        max_context_segs=args.max_context_segs, max_gap_to_target_secs=args.max_gap_to_target_secs)
    ...
    train_batch_sampler = LengthGroupedBatchSampler(train_sampler, compute_ctx_lens(train_ds, args.max_context_segs),
        batch_size=args.batch_size, drop_last=True)
    train_loader = DataLoader(train_ds, batch_sampler=train_batch_sampler, num_workers=args.num_workers,
        collate_fn=wm_collate_fn, pin_memory=pin, persistent_workers=args.num_workers > 0,
        prefetch_factor=2 if args.num_workers > 0 else None)
    val_batch_sampler = LengthGroupedBatchSampler(range(len(val_ds)), compute_ctx_lens(val_ds, args.max_context_segs),
        batch_size=args.batch_size, drop_last=False)
    val_loader = DataLoader(val_ds, batch_sampler=val_batch_sampler, num_workers=args.num_workers,
        collate_fn=wm_collate_fn, pin_memory=pin, persistent_workers=args.num_workers > 0,
        prefetch_factor=2 if args.num_workers > 0 else None)
    ...
    print(f"Total steps: {total_steps} ({steps_per_epoch}/epoch × {args.epochs} epochs)", flush=True)
