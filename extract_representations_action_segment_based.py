...
def extract_segments_for_video(video_path: str, annotation_entry: dict, processor, model, test: bool = False) -> Dict[int, dict]:
    """
    Extract one V-JEPA 2 feature per fine-grained action segment in this video.

    Returns: dict mapping segment_id -> {
        "features": np.ndarray (2048, 1024) float16,
        ...
        "noun": str,
        "correctness_raw": str,
        "sampled_frame_indices": np.ndarray (num_frames,) int32,
        "fps": float,
    }
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract V-JEPA-2 representations per HoloAssist fine-grained action segment."
    )
    parser.add_argument(
        "--annotation_path",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory containing HoloAssist videos. Each video is found at "
             "<video_dir>/<video_name>/<some>.mp4 — adjust find_video_path() to your layout.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        "--test",
        action="store_true",
        help="Stop after 5 segments (for quick CPU smoke-testing).",
    )
