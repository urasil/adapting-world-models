...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, attn_mask=None, key_padding_mask=None):
        B, N, C = x.shape
        ...
        if attn_mask is not None or self.use_sdpa:
            ...
            if key_padding_mask is not None:
                kpm_bias = torch.zeros(B, 1, 1, N, dtype=q.dtype, device=q.device).masked_fill(
                    key_padding_mask[:, None, None, :], float('-inf')
                )
                _attn_mask = kpm_bias if _attn_mask is None else _attn_mask + kpm_bias
            with torch.backends.cuda.sdp_kernel():
                x = F.scaled_dot_product_attention(
                    q, k, v, dropout_p=self.proj_drop_prob, is_causal=self.is_causal, attn_mask=_attn_mask
                )
                attn = None
        else:
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, attn_mask=None, T=None, H_patches=None, W_patches=None, key_padding_mask=None):
        if isinstance(self.attn, RoPEAttention):
            ...
            y = self.attn(self.norm1(x), mask=mask, attn_mask=attn_mask, key_padding_mask=key_padding_mask)
        x = x + self.drop_path(y)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, x, key_padding_mask=None):
        ...
        if self.use_sdpa:
            _attn_mask = None
            if key_padding_mask is not None:
                # float32 mask: avoids bf16 precision issues in softmax
                _attn_mask = torch.zeros(B, 1, 1, N, dtype=torch.float32, device=q.device).masked_fill(
                    key_padding_mask[:, None, None, :], float('-inf')
                )
            # Force math kernel: Flash Attention crashes with ~1M+ KV tokens.
            # Safe here because Q=1, so attention matrix is O(B×H×N) not O(N²).
            with torch.nn.attention.sdpa_kernel(torch.nn.attention.SDPBackend.MATH):
                q = F.scaled_dot_product_attention(q, k, v, attn_mask=_attn_mask)
        else:
            xattn = (q @ k.transpose(-2, -1)) * self.scale
            if key_padding_mask is not None:
                xattn = xattn.masked_fill(key_padding_mask[:, None, None, :], float('-inf'))
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, q, x, key_padding_mask=None):
        y = self.xattn(q, self.norm1(x), key_padding_mask=key_padding_mask)
        q = q + y
