...
# usage: python dev/smoke_test_wm.py --train_index ... --train_split ... --val_split ... --minutes 25
...
class Tee:
    # writes to both stdout and a file, flushing after every line
...
def grad_norm(model) -> tuple[float, bool]:
    # returns (total_norm, has_nan) for all parameter gradients
...
def main():
    ...
    def do_optimizer_step(note: str) -> tuple[float, bool]:
        # fires the optimizer, returns (grad_norm_val, has_nan_grad)
    ...
    try:
        while True:
            ...
            if (accum_full or (past_deadline and losses_window)):
                gn, has_nan = do_optimizer_step("scheduled" if accum_full else "forced-at-deadline")
                opt_steps += 1
